...
test("brand logo, favicon, profile shell, and footer marker follow the approved brand treatment", async () => {
  ...
  assert.match(footerSource, /kk-footer-logo-text/);
  assert.match(footerSource, /fontFamily:\s*"Manrope, sans-serif"/);
  assert.match(footerSource, /fontWeight:\s*800/);
  assert.match(footerSource, /letterSpacing:\s*"-2px"/);
  assert.match(footerSource, /linear-gradient\(135deg, #10b981 0%, #06b6d4 100%\)/);
  assert.match(footerSource, /WebkitBackgroundClip:\s*"text"/);
  assert.match(footerSource, /WebkitTextFillColor:\s*"#002a1b"/);
  assert.match(footerSource, /WebkitTextStroke:\s*"3px transparent"/);
  assert.doesNotMatch(footerSource, /kkFooterEmeraldGradient/);
  assert.doesNotMatch(footerSource, /stroke="url\(#kkFooterEmeraldGradient\)"/);
  assert.doesNotMatch(footerSource, /viewBox="0 0 800 400"/);
  assert.doesNotMatch(footerSource, /<svg[\s\S]*kk-footer-wordmark-title/);
  ...
  assert.match(profileShell, /kk-workspace-portal-label mt-2 pl-1\.5 font-label text-sm font-extrabold tracking-tight/);
  assert.doesNotMatch(profileShell, /kk-workspace-portal-label[^"]*uppercase/);
