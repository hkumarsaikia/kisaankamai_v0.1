...
test("rent equipment pages expose base search, query sort, and compact no-equipment spacing", async () => {
  ...
  assert.match(viewSource, /kk-rent-empty-shell/);
  assert.match(viewSource, /kk-rent-query-shell/);
  assert.match(viewSource, /kk-rent-available-shell/);
  assert.match(viewSource, /min-h-\[calc\(100svh-14rem\)\]/);
  assert.match(viewSource, /pb-8 pt-\[6\.25rem\]/);
  ...
  assert.doesNotMatch(viewSource, /pt-\[5\.5rem\][^"]*md:pt-\[5\.5rem\]/);
  assert.match(viewSource, /equipment-card-media-frame/);
  assert.match(viewSource, /p-2\.5 md:p-3/);
  assert.match(viewSource, /px-4 py-4 sm:px-6 lg:px-8/);
  assert.match(viewSource, /py-4 md:py-5/);
...
test("profile menu uses the supplied dropdown states and the trigger has a soft 3D visual boundary", async () => {
  ...
  assert.match(profileMenu, /w-\[min\(74vw,23\.5rem\)\]/);
  assert.match(profileMenu, /lg:w-\[23\.5rem\]/);
  ...
  assert.doesNotMatch(profileMenu, /\bw-80\b|w-\[20rem\]|w-\[min\((7[56]|8[06]|9[02])vw,(25|26|28|30)rem\)\]/);
