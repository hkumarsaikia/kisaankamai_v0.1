...
test("profile menu uses the supplied dropdown states and the trigger has a soft 3D visual boundary", async () => {
  ...
  assert.match(profileMenu, /w-\[min\(76vw,25rem\)\]/);
  assert.match(profileMenu, /lg:w-\[25rem\]/);
  ...
  assert.doesNotMatch(profileMenu, /\bw-80\b|w-\[20rem\]|w-\[min\((8[06]|9[02])vw,(26|28|30)rem\)\]/);
