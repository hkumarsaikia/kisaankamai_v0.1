...
const copy = {
  en: {
    eyebrow: "Feedback",
    title: "Share feedback for the marketplace",
    description: "Tell us what is working, what needs attention, and what would improve the experience next.",
    asideTitle: "How feedback is used",
    asideBody: "Feedback is reviewed alongside support requests so the team can prioritize the right fixes and improvements.",
    formTitle: "Send your feedback",
    formDescription: "Use the same structured fields each time so the review workflow stays predictable.",
    fullName: "Full name",
    ...
    messagePlaceholder: "Describe your experience in detail",
    topicPlaceholder: "e.g. Registration",
    subjectPlaceholder: "Brief subject",
  },
  mr: {
    eyebrow: "अभिप्राय",
    title: "मार्केटप्लेससाठी अभिप्राय द्या",
    description: "काय चांगले आहे, काय लक्ष देण्यासारखे आहे, आणि पुढे अनुभव सुधारण्यासाठी काय बदलावे ते सांगा.",
    asideTitle: "अभिप्राय कशासाठी वापरला जातो",
    asideBody: "अभिप्राय मदत विनंत्यांसोबत पाहिला जातो, त्यामुळे टीम योग्य सुधारणा प्राधान्याने करू शकते.",
    ...
    messagePlaceholder: "तुमचा अनुभव सविस्तर लिहा",
    topicPlaceholder: "उदा. नोंदणी",
    subjectPlaceholder: "संक्षिप्त शीर्षक",
  },
} as const;
...
export default async function FeedbackPage({
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 rounded-full border border-slate-200 bg-white/80 px-5 py-3 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
        <div className="text-xl font-black tracking-tight text-slate-950 dark:text-white">Kisan Kamai</div>
        <div className="flex items-center gap-3">
          <AppLink href="/support" className="rounded-full border border-soil-700 px-4 py-2 text-sm font-bold text-soil-700 transition hover:bg-soil-50 dark:border-soil-300 dark:text-soil-300 dark:hover:bg-soil-950/40">
            {page.support}
          </AppLink>
      ...
      <div className="mx-auto mt-12 max-w-3xl text-center">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-soil-700 dark:text-soil-300">{page.eyebrow}</p>
        <h1 className="mt-4 text-4xl font-black tracking-tight md:text-6xl">{page.title}</h1>
        <p className="mt-4 text-lg text-slate-600 dark:text-slate-300">{page.description}</p>
      </div>

      <div className="mt-12 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-6">
          <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[1.75rem] border border-slate-200 bg-white/85 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/85">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-soil-700 dark:text-soil-300">
                {locale === "mr" ? "फायरस्टोअर" : "Firestore"}
              </p>
          ...
          <form action={submitFeedbackAction} className="grid gap-5">
            <FormGrid>
              ...
              <FormField label={page.contact} required>
                <input className="kk-input" name="contact" placeholder={page.contact} required />
              </FormField>
            </FormGrid>
            <FormGrid>
              <FormField label={page.topic} required>
                <input className="kk-input" name="topic" placeholder={page.topicPlaceholder} required />
              </FormField>
              <FormField label={page.subject} required>
                <input className="kk-input" name="subject" placeholder={page.subjectPlaceholder} required />
              </FormField>
            </FormGrid>
            ...
            <FormField label={page.message} required>
              <textarea className="kk-input min-h-[180px]" name="message" placeholder={page.messagePlaceholder} required />
            </FormField>
