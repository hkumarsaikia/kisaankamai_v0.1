...
export function FormField({
  ...
  return (
    <div className="kk-form-field">
      ...
      {children}
      {error ? <p className="kk-form-error">{error}</p> : hint ? <p className="kk-form-hint">{hint}</p> : null}
    </div>
  );
}
...
export function FormNotice({
  ...
  return <Alert variant={tone}>{children}</Alert>;
}
...
export function FormChoicePills<T extends string>({
  options,
  value,
  onChange,
}: {
