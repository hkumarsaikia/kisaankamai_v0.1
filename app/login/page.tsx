...
import { DEMO_AUTH_CONFIG, getDemoLoginNote, isDemoLoginCredentials, startDemoSession } from "@/lib/demoAuth";
import { resolveLoginEmail } from "@/lib/teamReview";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isDemoMode) {
        if (!isDemoLoginCredentials(identifier, password)) {
          setError(
            langText(
              `Demo credentials only. ${getDemoLoginNote()}`,
              `फक्त demo credentials वापरा. ${getDemoLoginNote()}`
            )
          );
          return;
        }
  ...
  const handleGoogleLogin = async () => {
    if (isDemoMode) {
      setError(
        langText(
          "Google sign-in is disabled in demo mode. Use the shared demo credentials instead.",
          "Demo mode मध्ये Google sign-in बंद आहे. त्याऐवजी shared demo credentials वापरा."
        )
      );
      return;
    }
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-12rem)] max-w-7xl items-center gap-12 px-6 py-16 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <section className="kk-glass mx-auto w-full max-w-xl p-8 lg:p-10">
            <div className="mb-8">
              ...
              {isDemoMode ? (
                <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-200">
                  {getDemoLoginNote()}
                </div>
              ) : null}
            </div>
