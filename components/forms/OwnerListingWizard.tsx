...
export function OwnerListingWizard({
  ...
  return (
    <FormShell
      ...
      description="This flow keeps the original listing fields while using the current Firebase-backed save path, review state, and image previews."
      step={step}
      totalSteps={3}
      aside={
        <div className="space-y-4">
          <h3 className="text-lg font-black text-primary">Owner checklist</h3>
          <ul className="space-y-3 text-sm font-medium text-on-surface-variant">
            <li>Listings save into the live Firebase-backed catalog.</li>
            <li>Image uploads use the current production media pipeline.</li>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {step === 2 ? (
          <FormSection title="Coverage, tags, and media" description="Location, supported work types, and media all stay wired to the same existing listing schema.">
            ...
            <FormField label="Listing images" hint="New uploads are added through the same production upload pipeline already used by the owner listing backend.">
        ...
        {step === 3 ? (
          <FormSection title="Review and publish" description="The review step keeps the original progressive confirmation flow while preserving the current submission contract.">
            <ReviewList
              items={[
